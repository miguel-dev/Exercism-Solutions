...
export class TranslationService {
   ...
   */
  free(text) {
    return this.api.fetch(text).then(translatedText => translatedText.translation);
  }
   ...
   */
  request(text) {
    const requestTranslation = text => new Promise(
      (resolve, reject) => this.api.request(
        text, error => (error ? reject(error) : resolve())
      )
    );
    return requestTranslation(text)
      .catch(() => requestTranslation(text))
      .catch(() => requestTranslation(text));
  }
  ...
  premium(text, minimumQuality) {
    return this.api.fetch(text)
      ...
      .then(translatedText => { 
        if (translatedText.quality < minimumQuality) {
